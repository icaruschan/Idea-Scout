...
> Last updated: **March 28, 2026** — verified against live workspace via MCP + automation code inspection.
...
> **Automation note:** The Trend Scout pipeline reads this DB via `dataSources.query` filtered to `⭐ Rating = ⭐⭐⭐⭐⭐ OR ⭐⭐⭐⭐` (top 15 posts). Fields `Steal-able Pattern` and `Tweet Structure` are passed verbatim to the LLM as templates.
...
**Role:** Synthesis hub — where patterns become content ideas

| Property                   | Type                              | Description                                                                                                                                   |
| -------------------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `Idea`                     | title                             | The concept                                                                                                                                   |
| `Source`                   | select                            | `Idea Scout` _(automation)_, `Trend Scout` _(legacy)_, `Library Pattern` _(legacy)_, `My Winner`, `AI Generated`, `Random Thought`             |
| `Status`                   | select                            | `💭 Raw`, `✏️ Developed`, `📅 Scheduled`, `✅ Used`                                                                                           |
| `Priority`                 | select                            | `🔥 Hot`, `💡 Good`, `📝 Maybe` — assigned by LLM                                                                                             |
...
| `Inspired By (Library)`    | **relation → Viral Post Library** | Lineage tracking (legacy Pattern Miner populated this)                                                                                        |
| `Inspired By (My Content)` | **relation → My Content Tracker** | Lineage tracking                                                                                                                              |

> **Automation note:** The Consolidated Idea Scout pipeline writes 5-8 raw ideas per run. It sets `Status = 💭 Raw` and `Source = "Idea Scout"`. The page body contains the source context recap, rough draft copy, why it works explanation, and patterns used. Relations are established to both the Scouted Content (for lineage) and the Viral Post Library pattern used.

**Views (live):**

- `Ready to Use` — table, filter: Status = ✏️ Developed AND Status ≠ ✅ Used
